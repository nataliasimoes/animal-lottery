...
export class AppService {
    ...
    this.cachedItem.subscribe((result) => {
      const r: Result = {
        animal: lottery.find((animal) =>
          animal.dozens.includes(result.substring(2, 4))
        ),
        number: result,
      };
  ...
  // retorna um observable que emite um valor a cada 5 segundos de um número aleatório da milhar
  private cachedItem = interval(5000).pipe(
    map(() => {
